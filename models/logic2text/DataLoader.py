...
class Dataloader(object):
    def __init__(self, train_name, val_name, test_name):
        ...
        if test_name:
            with open(test_name) as f:
                self.test = None#json.load(f)
            self.test_ids = None#list(self.test.keys())
    ...
    def obtain_idx(self, idx, option):
        if option == 'train':
            ...
            table_id = None #self.test_ids[idx]
            entry = None #self.test[table_id]
        else:
...
class GPTTableDatabase(Dataloader):
    ...
    def get_data(self, datasource, cols: list, table_description, option='train', details=False):
        
        d = datasource #pandas.read_csv('data/gpt2_input.csv', sep=',')
        ...
        tmp_prefix = self.tokenizer.tokenize('Given the table title "{}" . '.format(table_description))
    ...
    def get_data_for_table(self, table_data, cols: list, details=False):
        ...
        if details:
            ...
            return descs, table_data,  # Add other details if necessary
        else:
            # Return only the generated text (descriptions)
            return descs
