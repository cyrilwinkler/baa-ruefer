...
def sample_sequence(model, length, context, args, num_samples=1, temperature=1, stop_token=None, \
                    trigger=None, supress=None, repetition=None, top_k=0, top_p=0.0,  device='mps'):
    #print(f'context_type: {type(context)}')
    ...
    with torch.no_grad():
        for _ in range(length):
            ...
            if isinstance(outputs, list) or isinstance(outputs, tuple):
                next_token_logits = outputs[0].logits[:, -1, :] / (temperature if temperature > 0 else 1.)
            else:
