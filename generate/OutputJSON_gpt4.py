...
class OutputJSON():
    ...
    :param gpt_api_key: api key for gpt-4
    ...
    def generate_gpt_flavor(self, input):
        """
        Generate the output with the gpt-4 model
    ...
    def format_to_json(self, input):
        """
        Format the output to a valid json format using gpt-4.
        :param input: input text
        :return: output text
        """
        client = OpenAI(
...
if __name__ == "__main__":
    ...
    print(f'save rouge scores')
    output_json.metrics.save_scores_to_json(path='', filename='scores.json')